...
type PlaceSuggestion = { id: string; label: string; mapsUrl: string };
...
export default function DriverCreateLoadScreen() {
  ...
  useEffect(() => {
    let isMounted = true;
    apiFetch<Truck[]>('/api/trucks', { method: 'GET' })
      .then((data) => { if (isMounted) setTrucks(data); })
      .catch(() => {});
    return () => { isMounted = false; };
  }, []);

  const fetchPlaces = async (query: string, which: 'pickup' | 'delivery') => {
    if (!query || query.trim().length < 3) {
      which === 'pickup' ? setPickupSuggestions([]) : setDeliverySuggestions([]);
      return;
    }
    ...
    try {
      ...
      which === 'pickup' ? setPickupSuggestions(results) : setDeliverySuggestions(results);
    } catch {
      which === 'pickup' ? setPickupSuggestions([]) : setDeliverySuggestions([]);
    } finally {
  ...
  const removeImage = () => { setLoadImageUrl(null); setLocalImageUri(null); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.replace('/(driver)/(tabs)/my-loads');
    } catch (error: any) {
      ...
      <ScrollView
        ...
        <View style={styles.form}>
          <View style={styles.field}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                ...
                pickupTimeoutRef.current = setTimeout(() => fetchPlaces(text, 'pickup'), 400);
              }}
              style={styles.input}
            ...
            {(pickupLoading || pickupSuggestions.length > 0) && (
              <View style={styles.suggestionsBox}>
                {pickupLoading && <Text style={styles.suggestionText}>Searching…</Text>}
          ...
          <View style={styles.field}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                ...
                deliveryTimeoutRef.current = setTimeout(() => fetchPlaces(text, 'delivery'), 400);
              }}
              style={styles.input}
            ...
            {(deliveryLoading || deliverySuggestions.length > 0) && (
              <View style={styles.suggestionsBox}>
                {deliveryLoading && <Text style={styles.suggestionText}>Searching…</Text>}
          ...
          <View style={styles.field}>
            ...
            {(localImageUri || loadImageUrl) ? (
              <View style={styles.imageBox}>
                ...
                <Pressable style={styles.removeImageBtn} onPress={removeImage} disabled={imageUploading}>
          ...
          <Pressable
            ...
            <Text style={styles.buttonText}>{saving ? 'Posting…' : 'Post load'}</Text>
          </Pressable>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  addImageBtn: { borderRadius: 12, borderWidth: 1, borderColor: '#D1D5DB', borderStyle: 'dashed', paddingVertical: 24, alignItems: 'center', backgroundColor: '#F9FAFB' },
  ...
  uploadingOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.4)', borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  uploadingText: { color: '#ffffff', fontWeight: '600', fontSize: 14 },
  removeImageBtn: { position: 'absolute', bottom: 8, right: 8, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: 'rgba(0,0,0,0.6)' },
