...
export const AuthProvider = ({children}) => {
  ...
  const setInitializing = isInitializing =>
    setState(prev => ({...prev, initializing: isInitializing}));
    ...
    try {
      ...
      if (auth.currentUser) {
        try {
          ...
          console.error(
            'Error cleaning up user after failed signup:',
            deleteError,
          );
        }
      }
  ...
  const signIn = async (email, password) => {
    try {
      ...
      if (
        ...
        console.log(
          'Blocking sign in for unverified driver email:',
          user.email,
        );
        try {
          await firebaseSignOut(auth);
  ...
  const resendVerificationEmail = async (email, password) => {
    try {
      ...
      try {
        ...
        console.error(
          'Error signing out after resendVerificationEmail:',
          signOutError,
        );
      }
      ...
      try {
        ...
        if (currentUser) {
          ...
          if (currentUser.isAnonymous) {
            console.log('Anonymous user detected — staying on signup flow');
            ...
            return; // ⛔ STOP here — no Firestore, no navigation
          }
          ...
          if (userData) {
            ...
            if (
              ...
              try {
                ...
                console.error(
                  'Error signing out unverified driver in listener:',
                  e,
                );
              }
            ...
            if (retryData) {
              ...
              try {
                ...
                console.error(
                  'Error saving to AsyncStorage after retry:',
                  storageError,
                );
              }
            } else {
              console.log(
                'No user data found after retry, keeping user signed in with fallback data.',
              );
              try {
                ...
                const fallbackUserType =
                  storedUserType || fallbackUser.userType || 'shipper';

                setUser(fallbackUser);
                setUserType(fallbackUserType);
              } catch (fallbackErr) {
                console.error(
                  'Error applying fallback user state:',
                  fallbackErr,
                );
