  ...
  const handleAcceptBid = async bid => {
    try {
      ...
      const shipmentData = {
        ...
        fareOffer: bid.offerAmount || 0, // ✅ Add default value
        status: 'pending',
        driverAccepted: true,
      ...
      const cleanShipmentData = Object.entries(shipmentData).reduce(
        (acc, [key, value]) => {
          if (value !== undefined) {
            acc[key] = value;
          }
      ...
      console.log('Clean shipment data:', cleanShipmentData); // Debug log

      const shipmentResult = await firebaseShipmentsService.createShipment(
        cleanShipmentData, // ✅ Use cleaned data
      );
